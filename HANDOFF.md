...
- 版本：**main = v15.00（已 push；GH Pages 依常規部署）**。localStorage schema 未變、向下相容。

## 本次 session（v15.00，已 commit+push、preview 實測通過、無 console error）＝待辦／筆記拆頁＋整理規則互斥＋雙向更換鈕
...
- 版本 vNN.MM：小改/修 bug 直接 bump minor；新功能大改先確認。目前 **v15.00**（APP_VERSION 與 sw CACHE_NAME 同步）。
